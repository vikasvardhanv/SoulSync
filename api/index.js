...
const initializeRoutes = async () => {
  try {
    ...
    console.log('Routes registered:', ['/auth', '/users', '/questions', '/matches', '/messages', '/subscriptions', '/payments', '/images']);
  } catch (error) {
...
// Initialize routes and setup 404 handler after routes are loaded
initializeRoutes().then(() => {
  // 404 handler for API routes - only set up after routes are loaded
  app.use('*', (req, res) => {
    res.status(404).json({
      success: false,
