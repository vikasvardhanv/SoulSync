...
const API_BASE_URL = import.meta.env.VITE_API_URL || 'https://soulsync.solutions/api';
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 413) {
      const message = error.response?.data?.message || 'File too large';
      const suggestion = error.response?.data?.error?.suggestion || 'Please use a smaller file.';
      
      toast.error(`${message} ${suggestion}`, {
        duration: 6000,
        style: {
          maxWidth: '400px',
        },
...
export const imagesAPI = {
  uploadSingle: (formData: FormData) => api.post('/images/upload', formData, {
    headers: { 'Content-Type': 'multipart/form-data' }
  }),
  uploadMultiple: (formData: FormData) => api.post('/images/upload-multiple', formData, {
    headers: { 'Content-Type': 'multipart/form-data' }
  }),
  deletePhoto: (data: { imageUrl: string }) => api.delete('/images/delete', { data }),
